...
const FullScreenModal: React.FC<FullScreenModalProps> = ({ short, isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchShorts = async () => {
      try {
        ...
        console.log('Shorts loaded:', data);
        setShorts(data);
        setLoading(false);
      } catch (err) {
...
const ShortCard: React.FC<ShortCardProps> = ({ short, index, onShortClick }) => {
  const videoRef = useRef<HTMLVideoElement>(null);
